...
\begin{center}
\hspace*{-1cm}
\includegraphics[scale=0.9]{prezzoMin.png}
\hspace*{-1cm}
\end{center}
		\item Funzione che restituisce il costo totale dell'ordine di un cliente.
\begin{lstlisting}[language=sql]
DELIMITER $$
CREATE FUNCTION `costoTotale`(`tot` DECIMAL(4,2), `email` VARCHAR(30)) RETURNS decimal(4,2)
...
\begin{center}
\hspace*{-1cm}
\includegraphics[scale=0.9]{costoTot.png}
\hspace*{-1cm}
\end{center}
\end{enumerate}
\end{document}

 ...
 k
	...
	\section{Progettazione concettuale}
		...
		\pagebreak
  raph{Molteplicità N:N} Un cliente registrato può rilasciare più feedback al ristorante, il ristorante può ottenere più feedback dai suoi clienti registrati.
	...
	\section{Progettazione logica}
		...
		\end{figure}
			...
			\subsubsection{Allergia}
				...
				\paragraph{Attributi}
					\begin{itemize}[noitemsep]
						\item Nome: \textit{string <<PK>>} - nome dell'allergia.
					\end{itemize}
			\paragraph{Cliente registrato-Allergia: "Appartiene"}
				\paragraph{Molteplicità N:N} Il cliente può avere più allergie, un'allergia può appartenere a più clienti.
				\paragraph{Totalità: parziale verso Allergia/ Totale verso Cliente registrato} Un cliente può non avere allergie, un'allergia appartiene ad almeno un cliente.
			\paragraph{Pietanza}
			L'entità Pientanza rappresenta l'insieme di pietanze che l'utente può ordinare. \newline
			Rispetto alla precedente entit\`a aggiungiamo gli attributi:
=======
 df6d0a84bc59fe383447733dfda2c59b2082dee9
a: \textit{string} - tipologia del cibo (i.e. vegana, vegetariana, [...])
		...
		\begin{itemize}[noitemsep]
			...
			\item[] \tex 
               	\pagebreak
n{Descrizione testuale dello schema relazionale}
1}) non ristrutturato possiamo fare alcune osservazioni relativamente alle entità \texttt{Utente} e \texttt{Pietanza}. In particolare, nell'analisi dei requisiti \`e specificato che un utente registrato pu\`o soffrire di una o pi\`u allergie, \`e pertanto necessario trasformare questo attributo \texttt{multivalore} \textit{Allergia} in un'entit\`a indipendente legata tramite una relazione \texttt{molti a molti}. Infatti, ogni utente pu\`o soffrire di una o pi\`u allergie e la stessa allergia potrebbe essere la medesima per pi\`u clienti. Per quanto riguarda l'entit\`a \textit{Pietanza} invece, introduciamo un attributo \texttt{Tipologia} che indica il tipo di pietanza (vegana, composta da carne, ecc), se questa pientanza apparterrà alle tipologia vegana allora sarà necessario conoscere le vitamine presenti nella pientanza. Pertanto, visto che la stessa vitamina potrebbe essere presente in più pietanze, anche l'attributo \textit{Vitamina} caratteristico della pietanza vegana diviene entità collegato alla relazione pietanza tramite una relazione \texttt{molti a molti}. 
{itemize}
nte registrato-Allergia: "Soffre"}
rire di più allergie, un'allergia può essere sofferta da più clienti.
allergia appartiene ad almeno un cliente.tbf{Ordine}(\underline{Codice}, Orario, Orario ordine, Orario Consegna, \textit{Cliente})

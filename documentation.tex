...
\hspace*{-1cm}
\end{center}
	\item Query che ritorna il codice fiscale del cliente che ha ordinato più volte la stessa pietanza
	\end{enumerate}
...
\end{center}
		\subsection{Funzioni}
		Funzione che restituisce il prezzo minimo o massimo delle pietanze ordinate.
		\begin{lstlisting}[language=sql]
CREATE FUNCTION `prezzoMinMax MinMax ENUM ('min','max')`(`cos` DECIMAL(4,2)) RETURNS DECIMAL(4,2) 
DETERMINISTIC 
CONTAINS SQL 
SQL SECURITY DEFINER 
IF(MinMax = 'min') THEN
	SELECT MIN(costo) into cos 
	FROM dettagli_ordine dor JOIN pietanza p on dor.pietanza=p.codice;
	RETURN cos;
ELSE
	SELECT MAX(costo) into cos 
	FROM dettagli_ordine dor JOIN pietanza p on dor.pietanza=p.codice;
	RETURN cos;
END IF;
\end{lstlisting}

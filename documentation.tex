...
\begin{center}
	...
	\end{table}
\end{center}
	\item Query che ritorna il codice fiscale del cliente che ha ordinato più volte la stessa pietanza in ordini diversi.
	\begin{lstlisting}[language=sql]
	\end{lstlisting}
	\item Query che ritorna l'email clienti che non hanno mai effettuato un ordine nonstante possano farlo (data\_di\_attivazione non nulla).
	...
	\begin{lstlisting}[language=sql]
SELECT email from cliente WHERE email NOT IN (SELECT email from cliente c JOIN ordine o ON c.email = o.cliente JOIN dettagli_ordine d_o ON o.codice = d_o.ordine WHERE c.data_di_attivazione IS NOT NULL)
	\end{lstlisting}	
	\end{enumerate}
...
\begin{center}
		\begin{table}[H]
		\centering
		\caption{Risultato della funzione 2}
		\label{table:query2}
		\begin{tabular}{|c|}
			\hline
